...
import { NewsModal, NotificationsModal, NewsItem, NotificationItem, NewsDetailModal } from './DashboardModals';
...
export function Dashboard({ user, notifications, onAction, onMarkAsRead, onMarkAllAsRead }: DashboardProps) {
  ...
  const displayQuickActions = isManager 
    ? (isBOD ? managerQuickActions : managerQuickActions.filter(a => a.id === 'team-status'))
    : staffQuickActions;
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-8">
        ...
        <button 
          ...
          className="px-6 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-xl font-bold transition-all shadow-[0_0_20px_rgba(59,130,246,0.3)]"
        >
          Try Again
        </button>
      </div>
  ...
  return (
    <>
      ...
      <NewsDetailModal isOpen={!!selectedNews} onClose={() => setSelectedNews(null)} item={selectedNews} />
      <NotificationsModal 
      ...
      <div className="flex flex-col gap-8 max-w-7xl mx-auto w-full animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <motion.div 
            ...
            className="order-1 lg:order-1 lg:col-span-2 relative overflow-hidden rounded-[2rem] bg-white/[0.03] backdrop-blur-2xl border border-white/10 shadow-[inset_0_0_30px_rgba(255,255,255,0.02)] flex flex-col p-6 md:p-8"
          >
                  ...
                  return (
                    <div key={item.id} className="news-item-container">
                      <div 
                        ...
                        className="p-5 rounded-2xl transition-all duration-300 group cursor-pointer border border-white/5 hover:bg-white/5 hover:-translate-y-0.5 hover:shadow-md hover:shadow-black/20"
                      >
                        <div className="flex flex-col gap-3">
                          <div className="flex items-center justify-between">
                            <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide bg-blue-500/20 text-blue-300">
                              {item.type}
                            </span>
                            <span className="text-[11px] font-medium text-slate-400">{item.date}</span>
                          </div>
                          
                          <h3 className="text-lg font-bold leading-tight transition-colors text-white group-hover:text-blue-300">
                            {item.title}
                          </h3>
          ...
          <div className="order-2 lg:order-2 lg:col-span-1 flex flex-col gap-6">
            <motion.div 
              ...
              className="relative overflow-hidden rounded-[2rem] bg-white/[0.03] backdrop-blur-2xl border border-white/10 shadow-[inset_0_0_30px_rgba(255,255,255,0.02)] flex flex-col p-6 md:p-8 flex-1"
            >
            ...
            {isManager && (
              <motion.div 
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="order-3 relative overflow-hidden rounded-[2rem] bg-white/[0.03] backdrop-blur-2xl border border-white/10 shadow-[inset_0_0_30px_rgba(255,255,255,0.02)] flex flex-col p-6 md:p-8"
              >
                 <div className="flex items-center justify-between mb-6">
        ...
        >
          {displayQuickActions.map((action) => (
            <button 
              ...
              className="flex flex-col items-center justify-center gap-4 p-6 md:p-8 rounded-[2rem] bg-white/[0.03] backdrop-blur-2xl border border-white/10 shadow-[inset_0_0_30px_rgba(255,255,255,0.02)] hover:border-white/20 hover:bg-white/10 transition-all group"
            >
